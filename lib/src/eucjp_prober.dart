...
class EUCJPProber extends CharSetProber {
  EUCJPProber(bool aIsPreferredLanguage)
    : _mIsPreferredLanguage = aIsPreferredLanguage,
      _mCodingSM = CodingStateMachine(
        model: MBCSStateMachineModels.kEUCJPModel,
      ) {
    reset();
  }
  ...
  final _mContextAnalyser = EUCJPContextAnalysis();

  final _mDistributionAnalyser = EUCJPDistributionAnalysis();
  ...
  ProberState feed(Uint8List aBuf) {
    ...
    for (var ii = 0; ii < aBuf.length; ii++) {
      ...
      if (codingState == eStart) {
        ...
        if (ii == 0) {
          _mLastChar[1] = aBuf[0];
          _mContextAnalyser.feedOneChar(_mLastChar.sublist(0, charLen));
          _mDistributionAnalyser.feedOneChar(_mLastChar.sublist(0, charLen));
        } else {
          _mContextAnalyser.feedOneChar(aBuf.sublist(ii - 1, ii - 1 + charLen));
